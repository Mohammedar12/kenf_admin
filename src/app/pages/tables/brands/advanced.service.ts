 ...
 */
function sort(tables: Brands[], column: string, direction: string): Brands[] {
    if (direction === '' || column === '') {
        return tables;
    } else {
 ...
 */
function matches(tables: Brands, term: string, pipe: PipeTransform) {
...
export class AdvancedService {
    ...
    // tslint:disable-next-line: variable-name
    private _tables$ = new BehaviorSubject<Brands[]>([]);
    // tslint:disable-next-line: variable-name
    ...
    private tableData: Brands[];

    constructor(private pipe: DecimalPipe,private sharedDataService: SharedDataBrandsService,) {
     ...
     * Search Method
     */
    private _search(): Observable<{ tables: Brands[]; total: number; }> {
